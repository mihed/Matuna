...
void OCLConvNet<T>::TrainNetwork(unique_ptr<ConvNetTrainer<T>> trainer,
	...
	accumulatorFiles.push_back(
			FileHelper::GetTextFromPath(
					Path::Combine(folderPath, programName)));
	...
	contexts[0]->AddProgramFromSource(programName, compilerOptions.str(),
			accumulatorFiles, oneDeviceVector);
		...
		for (int batch = 0; batch < batchIterations; batch++) {
			...
			for (int sample = 0; sample < batchSize; sample++) {
				...
				if (sample == 0) {
					...
					for (int i = 0; i < layerCount; i++) {
						...
						for (int j = 0; j < gradientsPointers[i].size(); j++) {
							accumulateKernel.SetAccumulator(
									accumulatedGradientsPointers[i][j]);
							accumulateKernel.SetInput(gradientsPointers[i][j]);
							accumulateKernel.SetGlobalWorkSize(
									parameterCount[j]);
			...
			for (int i = 0; i < layerCount; i++) {
				...
				for (int j = 0; j < accumulatedGradientsPointers[i].size();
					...
					accumulateScalarKernel.SetInput(
							accumulatedGradientsPointers[i][j]);
					accumulateScalarKernel.SetAccumulator(
							parametersToUpdate[j]);
