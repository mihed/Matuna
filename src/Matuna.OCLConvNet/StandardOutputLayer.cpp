...
namespace Matuna {
	namespace MachineLearning {
		...
		template<class T>
		void StandardOutputLayer<T>::InitializeImageProgram(unordered_map<OCLDevice*, unique_ptr<OCLProgram>>& programs)
		{
			auto inForwardPropMem = this->InForwardPropMemoryDescriptions()[0];
			auto inForwardPropData = this->InForwardPropDataDescriptions()[0];
			auto outBackPropMem = this->OutBackPropMemoryDescriptions()[0];

			string outputSourcePath =  Path::Combine(OCLProgram::DefaultSourceLocation, "ImageOutputBackProp.cl");
			string errorSourcePath =  Path::Combine(OCLProgram::DefaultSourceLocation, "ImageOutputError.cl");
			...
			{
				auto deviceInfo = device->DeviceInfo();
				auto maximumConstantBufferSizeOriginal = deviceInfo.MaxConstantBufferSize();
				...
				imageOutputKernel->AddDefineSubsitute(outputSourcePath, "INPUT_OFFSET_WIDTH", to_string(inputUnitMemoryWidthOffset));
				imageOutputKernel->AddDefineSubsitute(outputSourcePath, "INPUT_OFFSET_HEIGHT", to_string(inputUnitMemoryHeightOffset));
				imageOutputKernel->AddDefineSubsitute(outputSourcePath, "INPUT_UNIT_OFFSET", to_string(inputUnitOffset));

				imageOutputKernel->AddDefineSubsitute(outputSourcePath, "OUTPUT_OFFSET_WIDTH", to_string(outputUnitMemoryWidthOffset)); 
				imageOutputKernel->AddDefineSubsitute(outputSourcePath, "OUTPUT_OFFSET_HEIGHT", to_string(outputUnitMemoryHeightOffset)); 
				imageOutputKernel->AddDefineSubsitute(outputSourcePath, "OUTPUT_UNIT_OFFSET", to_string(outputUnitOffset));

				imageOutputKernel->AddDefineSubsitute(outputSourcePath, "INPUT_STRIDE", to_string(inputUnitMemoryWidth));
				imageOutputKernel->AddDefineSubsitute(outputSourcePath, "OUTPUT_STRIDE", to_string(outputUnitMemoryWidth));

				imageOutputKernel->AddDefineSubsitute(outputSourcePath, "OUTPUT_UNIT_ELEMENT_COUNT_INC_PADDING", to_string(outputUnitMemoryHeight * outputUnitMemoryWidth));
				imageOutputKernel->AddDefineSubsitute(outputSourcePath, "INPUT_UNIT_ELEMENT_COUNT_INC_PADDING", to_string(inputUnitMemoryHeight * inputUnitMemoryWidth));

				bool useBinary = false;
				if (globalWidth == 1 && globalHeight == 1 && globalUnits == 1)
					useBinary = true;

				SetErrorFunctionDefine(imageOutputKernel.get(), outputSourcePath, useBinary);
			...
			for (auto device : devices)
			{
				...
				imageErrorKernel->AddDefineSubsitute(errorSourcePath, "INPUT_OFFSET_WIDTH", to_string(inputUnitMemoryWidthOffset));
				imageErrorKernel->AddDefineSubsitute(errorSourcePath, "INPUT_WIDTH_LIMIT", to_string(inputUnitMemoryWidthOffset + dataWidth));
				imageErrorKernel->AddDefineSubsitute(errorSourcePath, "INPUT_HEIGHT_LIMIT", to_string(inputUnitMemoryHeightOffset + dataHeight));
				imageErrorKernel->AddDefineSubsitute(errorSourcePath, "INPUT_OFFSET_HEIGHT", to_string(inputUnitMemoryHeightOffset));
				imageErrorKernel->AddDefineSubsitute(errorSourcePath, "INPUT_UNIT_OFFSET", to_string(inputUnitOffset));
				imageErrorKernel->AddDefineSubsitute(errorSourcePath, "INPUT_STRIDE", to_string(inputUnitMemoryWidth));
				imageErrorKernel->AddDefineSubsitute(errorSourcePath, "INPUT_UNIT_LIMIT", to_string(inputUnitOffset + dataUnits));
				imageErrorKernel->AddDefineSubsitute(errorSourcePath, "INPUT_UNIT_ELEMENT_COUNT_INC_PADDING", to_string(inputUnitMemoryWidth * inputUnitMemoryHeight));
		...
		template<class T>
		void StandardOutputLayer<T>::InitializeProgram(unordered_map<OCLDevice*, unique_ptr<OCLProgram>>& programs)
		{
			auto inForwardPropMem = this->InForwardPropMemoryDescriptions()[0];
			auto outBackPropMem = this->OutBackPropMemoryDescriptions()[0];
			...
			string outputSourcePath =  Path::Combine(OCLProgram::DefaultSourceLocation, "OutputBackProp.cl");
			string errorSourcePath =  Path::Combine(OCLProgram::DefaultSourceLocation, "OutputError.cl");
			string includePath = OCLProgram::DefaultSourceLocation;
			auto errorFunction = config.ErrorFunction();
			...
			for (auto device : devices)
			{
				...
				outputKernel->AddDefineSubsitute(outputSourcePath, "INPUT_UNIT_OFFSET", to_string(inForwardPropMem.UnitOffset));
				outputKernel->AddDefineSubsitute(outputSourcePath, "OUTPUT_UNIT_OFFSET", to_string(outBackPropMem.UnitOffset));
			...
			for (auto device : devices)
			{
				...
				errorKernel->AddDefineSubsitute(errorSourcePath, "INPUT_UNIT_OFFSET", to_string(inForwardPropMem.UnitOffset));
				errorKernel->AddDefineSubsitute(errorSourcePath, "INPUT_COUNT", to_string(inputDescription.Units));
