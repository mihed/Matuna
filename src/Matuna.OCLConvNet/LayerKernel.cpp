...
template<class T>
void LayerKernel<T>::AddDefineSubsitute(string path, string defineName,
		int defineValue)
{
	InsertDefineSubstitute(path, defineName, Converter::ConvertToString(defineValue));
}
...
template<class T>
void LayerKernel<T>::InsertDefineSubstitute(string path, string defineName,
		string value)
{
	...
	if (map.find(defineName) != map.end())
		throw invalid_argument(
				"A define with that name has already been added");

	map.insert(make_pair(defineName, value));
}
...
void LayerKernel<T>::SetMemoryArg(OCLMemory* memory, int index)
{
	...
	CheckOCLError(
			clSetKernelArg(this->GetKernel(), index, sizeof(cl_mem),
					&rawMemory), "Could not set the kernel argument");
}
...
void LayerKernel<T>::SetRealArg(T value, int index)
{
	...
	CheckOCLError(clSetKernelArg(this->GetKernel(), index, sizeof(T), &value),
...
void LayerKernel<T>::SetLocalMemoryArg(size_t size, int index)
{
	...
	CheckOCLError(clSetKernelArg(this->GetKernel(), index, size, nullptr),
			"Could not set the kernel argument");
}
