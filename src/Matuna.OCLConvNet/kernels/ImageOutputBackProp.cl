	...
	__global TYPE* output)
{
	...
	output[outputIndex] = (tempInput - target[inputIndex]) * tempInput * (ONE - tempInput);
#elif defined(TANH)
	const TYPE tempInput = input[inputIndex];
	output[outputIndex] = (tempInput - target[inputIndex])  * TANH_INNER * (TANH_OUTER - (tempInput * tempInput) / TANH_OUTER);
#else
	output[outputIndex] = input[inputIndex] - target[inputIndex];
#endif
}
	...
	__global TYPE* output)
{
	...
	output[outputIndex] = input - target[inputIndex];
#elif defined(TANH)
	const TYPE input = inputs[inputIndex];
	output[outputIndex] = (input - target[inputIndex]) / (input * (ONE - input)) * TANH_INNER * (TANH_OUTER - (input * input) / TANH_OUTER);
#else
	const TYPE input = inputs[inputIndex];
	output[outputIndex] = (input - target[inputIndex]) / (input * (ONE - input));
#endif
}
...
#elif defined(TANH)
	const TYPE tempInput = input[inputIndex];
	output[outputIndex] = -target[inputIndex] / tempInput * TANH_INNER * (TANH_OUTER - (tempInput * tempInput) / TANH_OUTER);
#else
	output[outputIndex] = -target[inputIndex] / input[inputIndex];
#endif
}
