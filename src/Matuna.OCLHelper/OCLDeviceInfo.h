...
namespace Matuna {
	namespace Helper {
		...
		/**
		*@brief The full description of an OCLDevice
		*
		*This class contains all information necessary in order to
		*tune and execute kernels on a particular OCLDevice.
		*
		*/
		class OCLDeviceInfo final{
		...
		private:
			...
			/**
			*@brief Instantiate the OCLDeviceInfo.
		...
		public:
			...
			*/
			OCLPlatformInfo PlatformInfo() const {
				return platformInfo;
			}
			;
			...
			*@brief 
			*
			*The native ID to the device. Used internally inside OCLDevice in order to execute OCLKernel.
			*
			...
			*
			*CL_DEVICE_TYPE_CPU	An OCL device that is the host processor. The host processor runs the OCL implementations and is a single or multi-core CPU.
			*CL_DEVICE_TYPE_GPU	An OCL device that is a GPU. By this we mean that the device can also be used to accelerate a 3D API such as OpenGL or DirectX.
			*CL_DEVICE_TYPE_ACCELERATOR	Dedicated OCL accelerators (for example the IBM CELL Blade). These devices communicate with the host processor using a peripheral interconnect such as PCIe.
			*CL_DEVICE_TYPE_DEFAULT	The default OCL device in the system.
			*CL_DEVICE_TYPE_ALL	All OCL devices available in the system.
			*
			...
			*
			*The number of parallel compute units on the OCL device
			*
			...
			*
			*OCL software driver version string in the form major_number.minor_number
			*
			*@return OCL software driver version string in the form major_number.minor_number
			*/
			...
			*
			*OCL profile string. Returns the profile name supported by the device. The profile name returned can be one of the following strings:
			*FULL_PROFILE – if the device supports the OCL specification (functionality defined as part of the core specification and
			*does not require any extensions to be supported).
			*EMBEDDED_PROFILE - if the device supports the OCL embedded profile.
			*
			...
			*
			*OCL version string. Returns the OCL version supported by the device.
			*
			*@return OCL<space><major_version.minor_version><space><vendor-specific information>
			*/
			...
			/**
			*@brief CL_DEVICE_OCL_C_VERSION
			*
			*OCL C version string. Returns the highest OCL C version supported by the compiler for this device that is not of type CL_DEVICE_TYPE_CUSTOM.
			*
			*@return OCL<space>C<space><major_version.minor_version><space><vendor-specific information>
			*/
			string DeviceOCLVersion() const {
				return deviceOCLVersion;
			}
			;
