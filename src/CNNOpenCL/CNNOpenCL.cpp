...
namespace Matuna
{
	namespace MachineLearning
	{
			...
			const vector<OpenCLDeviceInfo>& deviceInfos)
		{
			...
			vector<
				tuple<OpenCLPlatformInfo,
				vector<tuple<OpenCLDeviceConfig, OpenCLDeviceInfo>>> > contextsConfigurations;
			...
			for (auto& contextConfig : contextsConfigurations)
				contexts.push_back(
				shared_ptr<OpenCLContext>(
				move(
				OpenCLHelper::GetContext(get<0>(contextConfig),
				get<1>(contextConfig)))));
		}
		...
		unique_ptr<T[]> CNNOpenCL<T>::FeedForwardAligned(T* input, int formatIndex)
		{
			...
			unique_ptr<OpenCLMemory> inputMemory = contexts[0]->CreateMemory(
		...
		T CNNOpenCL<T>::CalculateErrorAligned(T* input, int formatIndex, T* target)
		{
			...
			unique_ptr<OpenCLMemory> targetMemory = contexts[0]->CreateMemory(
		...
		template<class T>
		unique_ptr<T[]> CNNOpenCL<T>::BackPropAligned(T* input, int formatIndex,
			T* target)
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
		...
		template<class T>
		T CNNOpenCL<T>::CalculateErrorFromForwardAligned(T* propagatedValue,
			int formatIndex, T* target)
		{
			...
			device->WriteMemory(targetMemory.get(), targetMemory->ByteSize(), target, 0,
				true);
			unique_ptr<OpenCLMemory> propagatedMemory = contexts[0]->CreateMemory(
				CL_MEM_READ_ONLY, sizeof(T) * outputDescription.TotalMemory());
			...
			int formatIndex, T* target)
		{
			...
			if (layers.size() != 0)
			{
				...
				unique_ptr<OpenCLMemory> gradientMemory = contexts[0]->CreateMemory(
			...
			unique_ptr<IAlgorithmConfig> algorithm)
		{
			...
			vector<vector<unique_ptr<OpenCLMemory>>> gradients;
			vector<vector<OpenCLMemory*>> gradientsPointers;
			vector<vector<unique_ptr<OpenCLMemory>>> accumulatedGradients;
			vector<vector<OpenCLMemory*>> accumulatedGradientsPointers;

			for (int i = 0; i < layerCount; i++)
			{
				gradients.push_back(vector<unique_ptr<OpenCLMemory>>());
				accumulatedGradients.push_back(vector<unique_ptr<OpenCLMemory>>());
				gradientsPointers.push_back(vector<OpenCLMemory*>());
				accumulatedGradientsPointers.push_back(vector<OpenCLMemory*>());

				vector<size_t> parameterCount = layers[i]->GetMultipleParameterCount();
				for (int k = 0; k < parameterCount.size(); k++)
				{
					unique_ptr<OpenCLMemory> tempGradientMemory1 =
						contexts[0]->CreateMemory(
						CL_MEM_READ_WRITE, sizeof(T) * parameterCount[k]);
					unique_ptr<OpenCLMemory> tempGradientMemory2 =
						contexts[0]->CreateMemory(
						CL_MEM_READ_WRITE, sizeof(T) * parameterCount[k]);
			...
			for (int epoch = 0; epoch < epochs; epoch++)
			{
				...
				for (int batch = 0; batch < batchIterations; batch++)
				{
					...
					for (int sample = 0; sample < batchSize; sample++)
					{
						...
						unique_ptr<OpenCLMemory> backPropOutputMemory =
							contexts[0]->CreateMemory(
							CL_MEM_READ_ONLY,
							sizeof(T)
						...
						for (int i = layerCount - 1; i >= 1; i--)
						{
							...
							unique_ptr<OpenCLMemory> outputMemory =
								contexts[0]->CreateMemory(
								CL_MEM_READ_WRITE,
								sizeof(T)
					...
					for (int i = 0; i < layerCount; i++)
					{
						vector<OpenCLMemory*> parametersToUpdate = layers[i]->GetParameters();
						vector<size_t> parameterCount = layers[i]->GetMultipleParameterCount();
