...
template<class T>
void AccumulateVectorScalarKernel<T>::SetInput(OpenCLMemory* input)
{
...
template<class T>
void AccumulateVectorScalarKernel<T>::SetAccumulator(OpenCLMemory* accumulator)
{
...
{
	CheckOpenCLError(clSetKernelArg(this->GetKernel(), 2, sizeof(T), &scalar),
			"Could not set the kernel arguments");
}
