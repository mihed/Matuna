...
void SumAllUnitsKernel<T>::InitializeCompilerOptions()
{
	...
	stringStream << "-D" << "UNIT_COUNT_INC_PADDING=" << (inputUnitOffset + unitsToSum) << " ";
...
template<class T>
void SumAllUnitsKernel<T>::SetInput(OpenCLMemory* input)
{
...
template<class T>
void SumAllUnitsKernel<T>::SetOutput(OpenCLMemory* output)
{
