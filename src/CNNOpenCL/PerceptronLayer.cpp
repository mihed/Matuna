...
namespace ATML {
	namespace MachineLearning {
		...
		void PerceptronLayer<T>::InitializeImageBackPerceptron() {
			...
			LayerDataDescription outForwardDataDesc = this->outForwardPropDataDescriptions[0];
			LayerDataDescription inForwardDataDesc = this->InForwardPropDataDescriptions()[0];
			LayerMemoryDescription inBackMemDesc =  this->InBackPropMemoryDescriptions()[0];
			LayerMemoryDescription inForwardMemDesc =  this->InForwardPropMemoryDescriptions()[0];
			LayerMemoryDescription outBackMemDesc = this->OutBackPropMemoryDescriptions()[0];
		...
		void PerceptronLayer<T>::InitializeImageForwardPerceptron() 
		{
			...
			LayerMemoryDescription firstInputMemory = this->InForwardPropMemoryDescriptions()[0];
			...
			for (auto device : devices) {
				...
				unique_ptr<ImageForwardPerceptronKernel<T>> kernel(
					new ImageForwardPerceptronKernel<T>(biasCount, firstInputData.Units, firstInputData.Width, firstInputData.Height,
					firstInputMemory.WidthOffset, firstInputMemory.HeightOffset, firstInputMemory.UnitOffset, firstInputData.Width,
