...
#include "ConvNetOCL.h"
#include "ConvNetOCLFactoryVisitor.h"
...
namespace Matuna
{
	namespace MachineLearning
	{
		...
		template<class T>
		ConvNetOCL<T>::ConvNetOCL(const vector<OCLDeviceInfo>& devices,
		...
		TrainableConvNet<T>(*config)
		{
			...
			ConvNetOCLFactoryVisitor<T> factory(contexts[0], this);
		...
		template<class T>
		ConvNetOCL<T>::~ConvNetOCL()
		{

		}

		template<class T>
		void ConvNetOCL<T>::InitializeContexts(
		...
		template<class T>
		vector<OCLContext*> ConvNetOCL<T>::GetOCLContexts() const
		{
		...
		template<class T>
		unique_ptr<T[]> ConvNetOCL<T>::FeedForwardAligned(T* input, int formatIndex)
		{
		...
		template<class T>
		T ConvNetOCL<T>::CalculateErrorAligned(T* input, int formatIndex, T* target)
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
			auto device = devices[0];
		...
		template<class T>
		unique_ptr<T[]> ConvNetOCL<T>::BackPropAligned(T* input, int formatIndex,
			T* target)
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
		...
		template<class T>
		T ConvNetOCL<T>::CalculateErrorFromForwardAligned(T* propagatedValue,
		...
		template<class T>
		unique_ptr<T[]> ConvNetOCL<T>::CalculateGradientAligned(T* input,
			int formatIndex, T* target)
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
		...
		template<class T>
		unique_ptr<T[]> ConvNetOCL<T>::GetParameters()
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
			auto device = devices[0];
		...
		template<class T>
		void ConvNetOCL<T>::SetParameters(T* parameters)
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
			auto device = devices[0];
		...
		template<class T>
		size_t ConvNetOCL<T>::GetParameterCount()
		{
			size_t result = 0;
			for (auto& layer : layers)
		...
		template<class T>
		void ConvNetOCL<T>::TrainNetwork(unique_ptr<ConvNetTrainer<T>> trainer,
		...
		template<class T>
		vector<OCLForwardBackPropLayer<T>*> ConvNetOCL<T>::GetLayers() const
		{
		...
		template<class T>
		StandardOutputLayer<T>* ConvNetOCL<T>::GetOutputLayer() const
		{
			return outputLayer.get();
		}

		template class ConvNetOCL<cl_float> ;
		template class ConvNetOCL<cl_double> ;
