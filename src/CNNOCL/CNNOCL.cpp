...
namespace Matuna
{
	namespace MachineLearning
	{
		...
		template<class T>
		CNNOCL<T>::CNNOCL(const vector<OCLDeviceInfo>& devices,
		...
		TrainableCNN<T>(*config)
		{
			...
			CNNOCLFactoryVisitor<T> factory(contexts[0], this);
		...
		template<class T>
		void CNNOCL<T>::InitializeContexts(
		...
		template<class T>
		vector<OCLContext*> CNNOCL<T>::GetOCLContexts() const
		{
		...
		template<class T>
		unique_ptr<T[]> CNNOCL<T>::FeedForwardAligned(T* input, int formatIndex)
		{
		...
		template<class T>
		T CNNOCL<T>::CalculateErrorAligned(T* input, int formatIndex, T* target)
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
			auto device = devices[0];
		...
		template<class T>
		unique_ptr<T[]> CNNOCL<T>::BackPropAligned(T* input, int formatIndex,
			T* target)
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
		...
		template<class T>
		T CNNOCL<T>::CalculateErrorFromForwardAligned(T* propagatedValue,
		...
		template<class T>
		unique_ptr<T[]> CNNOCL<T>::CalculateGradientAligned(T* input,
			int formatIndex, T* target)
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
		...
		template<class T>
		unique_ptr<T[]> CNNOCL<T>::GetParameters()
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
			auto device = devices[0];
		...
		template<class T>
		void CNNOCL<T>::SetParameters(T* parameters)
		{
			//TODO: At the moment we only support one context and one device
			auto devices = contexts[0]->GetDevices();
			auto device = devices[0];
		...
		template<class T>
		size_t CNNOCL<T>::GetParameterCount()
		{
			size_t result = 0;
			for (auto& layer : layers)
		...
		template<class T>
		void CNNOCL<T>::TrainNetwork(unique_ptr<CNNTrainer<T>> trainer,
		...
		template<class T>
		vector<OCLForwardBackPropLayer<T>*> CNNOCL<T>::GetLayers() const
		{
		...
		template<class T>
		StandardOutputLayer<T>* CNNOCL<T>::GetOutputLayer() const
		{
			return outputLayer.get();
		}

		template class CNNOCL<cl_float> ;
		template class CNNOCL<cl_double> ;
