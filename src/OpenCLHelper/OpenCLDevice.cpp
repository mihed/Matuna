...
bool OpenCLDevice::ProgramAdded(string programName) {
	if (programsAndKernels.find(programName) == programsAndKernels.end())
		return false;
	else
		return true;
}
bool OpenCLDevice::KernelAdded(string programName, string kernelName) {
	if (programsAndKernels.find(programName) == programsAndKernels.end())
		return false;
	else {
...
unique_ptr<OpenCLMemory> OpenCLDevice::CreateMemory(cl_mem_flags flags, size_t bytes)
{
	cl_int error;
...
unique_ptr<OpenCLMemory> OpenCLDevice::CreateMemory(cl_mem_flags flags, size_t bytes, void* buffer)
{
	cl_int error;
