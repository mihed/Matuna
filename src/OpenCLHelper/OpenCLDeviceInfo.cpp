...
namespace ATML {
	namespace Helper {
		...
		platformInfo(platformInfo), deviceID(deviceID), globalMemoryCacheType(globalMemoryCacheType), localMemoryType(localMemoryType),type(type), vendorID(
			...
			maxConstantArguments), localMemorySize(
			...
			deviceOpenCLVersion), deviceExtensions(deviceExtensions) {
