...
SCENARIO("Creating kernels from the context", "[OCLHelper][OCLContext][OCLKernel][OCLKernelProgram]")
{
	auto platformInfos = OCLHelper::GetPlatformInfos();
	if (platformInfos.size() == 0)
	{
	...
	GIVEN("A OCLKernelProgram implementation")
	{
		TestKernel kernel;
		...
		auto context = OCLHelper::GetContext(platformInfo);
		vector<OCLDevice*> oneDevice;
		oneDevice.push_back(context->GetDevices()[0]);
		auto testKernel = context->CreateOCLKernelProgram<TestKernel>(oneDevice);
...
SCENARIO("Executing an OCL kernel", "[OCLDevice][OCLHelper][OCLKernel][OCLMemory]")
{
	auto platformInfos = OCLHelper::GetPlatformInfos();
	if (platformInfos.size() == 0)
	{
	...
	for (auto& platformInfo : platformInfos)
	{
...

		{
			...
			shared_ptr<OCLMemory> input1Memory(move(context->CreateMemory(CL_MEM_READ_ONLY, sizeof(cl_float) * bufferSize)));
			shared_ptr<OCLMemory> input2Memory(move(context->CreateMemory(CL_MEM_READ_ONLY, sizeof(cl_float) * bufferSize)));
			shared_ptr<OCLMemory> outputMemory(move(context->CreateMemory(CL_MEM_WRITE_ONLY, sizeof(cl_float) * bufferSize)));
