/*
* OCLConvNetForwardVanillaTest.cpp
*
...
unique_ptr<ConvNetConfig> CreateRandomConvNetVanillaConvolutionConfig(mt19937& mt,
																	  uniform_int_distribution<int>& layerGenerator,
																	  uniform_int_distribution<int>& dimensionGenerator,
																	  uniform_int_distribution<int>& unitGenerator,
																	  ...
																	  uniform_int_distribution<int>& samplingSizeGenerator)
{
	...
	for (int i = 0; i < layerCount; i++)
	{
		...
		unique_ptr<ForwardBackPropLayerConfig> vanillaConfig(new VanillaSamplingLayerConfig(widthSampling, heightSampling));
...
SCENARIO("Creating a vanilla sampling network with divisable layers")
{
	...
	for (int dummy = 0; dummy < 10; dummy++)
	{
		...
		{
			unique_ptr<ConvNetConfig> config = CreateRandomConvNetVanillaConfig(mt, layerGenerator, dimensionGenerator, unitGenerator, samplingSizeGenerator, true);
...
SCENARIO("Creating a vanilla sampling network with undivisable layers")
{
	...
	for (int dummy = 0; dummy < 10; dummy++)
	{
		...
		for (auto& deviceInfo : deviceInfos)
		{
			unique_ptr<ConvNetConfig> config = CreateRandomConvNetVanillaConfig(mt, layerGenerator, dimensionGenerator, unitGenerator, samplingSizeGenerator, false);
...
SCENARIO("Creating a vanilla sampling network combined with convolution")
{
	auto platformInfos = OCLHelper::GetPlatformInfos();
	random_device device;
	mt19937 mt(device());
	...
	for (int dummy = 0; dummy < 10; dummy++)
	{
		...
		for (auto& deviceInfo : deviceInfos)
		{
			...
			unique_ptr<ConvNetConfig> config = CreateRandomConvNetVanillaConvolutionConfig(mt, layerGenerator, dimensionGenerator, unitGenerator, filterGenerator, samplingSizeGenerator);
			OCLConvNet<float> network(deviceInfo, move(config));
			...
			for (auto layer : layers)
			{
				...
				{
					CHECK(dynamic_cast<VanillaSamplingLayer<float>*>(layer));
					auto config = dynamic_cast<VanillaSamplingLayer<float>*>(layer)->GetConfig();
					samplingSizes.push_back(make_tuple(config.SamplingSizeWidth(), config.SamplingSizeHeight()));
				}
			}
			...
			for (size_t i = 0; i < count; i++)
			{
				...
				for (size_t j = 0; j < tempFilters.size(); j++)
				{
					...
					tempResult = tempResult.VanillaDownSample(get<0>(samplingSize), get<1>(samplingSize));
