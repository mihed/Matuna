		...
		public:
			...
			*/
			...
			unique_ptr<T> CreateOpenCLKernelProgram(
				const vector<OpenCLDevice*>& affectedDevices)
			{
				static_assert(is_base_of<OpenCLKernel, OpenCLKernelProgram>::value, "The OpenCLKernelProgram is not a derived class of OpenCLKernel");
				...
				if (programs.find(result->ProgramName()) != programs.end())
					...
					"The program where the kernel belongs has already been added.");
